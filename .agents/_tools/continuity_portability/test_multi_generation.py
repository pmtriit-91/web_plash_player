#!/usr/bin/env python3
"""Executable W6 shard for multi-generation portability."""
...
PROOFS = (
    (
        ...
        "continuity_portability/test_migration_chain.py",
        10,
    ),
    (
        "atomic-multi-hop-apply-rollback",
        "continuity_transactions/test_migration_failure_contracts.py",
        6,
    ),
    (
        "export-generation-extension-binding",
        "continuity_portability/test_export_generation_extension.py",
        10,
    ),
    (
        "restore-generation-path-binding",
        "continuity_portability_restore/test_generation_path_extension.py",
        10,
    ),
)
...
def load_proof(path: Path, proof_id: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load multi-generation proof: {path}")

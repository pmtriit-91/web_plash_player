#!/usr/bin/env python3
"""Executable W6 shard for target-Windows path collision safety."""
...
PROOFS = (
    (
        "export-admission",
        "continuity_portability/test_windows_path_export.py",
        7,
    ),
    (
        "archive-admission",
        "continuity_portability/test_windows_path_archive.py",
        8,
    ),
    (
        "restore-admission",
        "continuity_portability/test_windows_path_restore.py",
        10,
    ),
)
...
def load_proof(path: Path, proof_id: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load Windows-path proof: {path}")

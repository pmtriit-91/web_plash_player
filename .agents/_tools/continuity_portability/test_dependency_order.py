#!/usr/bin/env python3
"""Executable W6 shard for dependency-ordered restore."""
...
PROOFS = (
    (
        "dependency-order-primitive",
        "continuity_portability/test_dependency_order_primitive.py",
        10,
    ),
    (
        "dependency-plan-contract",
        "continuity_portability/test_dependency_plan.py",
        10,
    ),
    (
        "dependency-apply-boundary",
        "continuity_portability/test_dependency_apply.py",
        8,
    ),
)
...
def load_proof(path: Path, proof_id: str) -> ModuleType:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load dependency-order proof: {path}")
